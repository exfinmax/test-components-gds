...
func _ready() -> void:
	...
	pivot_offset = size/2
	# 监听对话行变化信号，自动更新焦点
	#DialogueManager.dialogue_line_change.connect(_on_dialogue_line_change)
...
func _on_dialogue_line_change(dialogue_line: DialogueLine) -> void:
	"""监听对话行变化，自动更新焦点"""
	pass
	"""切换立绘，key 为表情键名（如 ax/ex/hp 等）"""
	if lihui_resource == null:
		return
	# 使用 has_expression 安全检查（若 LiHui 已扩展）
	if lihui_resource.has_method("has_expression"):
		if not lihui_resource.has_expression(key):
			return
	elif not lihui_resource.sprites.has(key):
		return
	texture = lihui_resource.sprites[key]
