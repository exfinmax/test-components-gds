...
func _ready():
    ...
    # recursive helper to spawn scenes from a folder
    func spawn_from_dir(dirpath:String, x_offset:float, y_offset:float):
        ...
        while true:
            ...
            if dir.current_is_dir():
                spawn_from_dir(full, x_offset + x*200, y_offset + y*150)
                ...
                if scene:
                    ...
                    if x > 3:
                        x = 0
                        y += 1
    # start recursion at templates root
    spawn_from_dir("res://ComponentLibrary/Packs/VFX/Templates", 0, 0)
