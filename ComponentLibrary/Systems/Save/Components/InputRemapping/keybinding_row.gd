...
##   └── BindButton   (Button)  — unique_name_in_owner
##
...
func setup(p_action: String, p_display: String, capture_dialog: KeyCaptureDialog) -> void:
	...
	_bind_button.pressed.connect(_on_bind_button_pressed)
	refresh()

## 直接从 InputMap 读取当前绑定刷新按钮文字
## ⚠ 不依赖 SaveSystem，InputMap 全局随时可用，无初始化时序问题
func refresh() -> void:
	if not is_instance_valid(_bind_button):
		return
	...
	if InputMap.has_action(action):
		...
		if events.size() > 0:
			ev = events[0]
	_bind_button.text = ResourceSerializer.event_to_display_string(ev)
...
func _on_key_captured(event: InputEvent, captured_action: String) -> void:
	...
	if km:
		km.rebind_action_primary(action, event)
		...
		if InputMap.has_action(action):
			...
			if old_events.size() > 0:
				InputMap.action_erase_event(action, old_events[0])
