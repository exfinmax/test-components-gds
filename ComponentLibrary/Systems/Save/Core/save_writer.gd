...
## 返回 true 表示成功
static func write_json(payload: Dictionary, path: String, game_version: String = "") -> bool:
	_ensure_dir(path)
	var envelope := {
		"_meta": {
...
## 一步完成：collect + write
## modules: Array[ISaveModule]
static func write(modules: Array, path: String, game_version: String = "") -> bool:
	var payload := collect(modules)
	return write_json(payload, path, game_version)
...
## modules: Array[ISaveModule]
static func read(path: String, modules: Array) -> bool:
