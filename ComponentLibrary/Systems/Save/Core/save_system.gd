...
const _MODULES_DIR_FALLBACK := "res://ComponentLibrary/Systems/Save/Modules"
...
func save_global() -> bool:
	var ok := SaveWriter.write(_global_modules.values(), _GLOBAL_PATH, game_version)
	global_saved.emit(ok)
	return ok

## 从 global.json 加载所有全局模块
func load_global() -> bool:
	var ok := SaveWriter.read(_GLOBAL_PATH, _global_modules.values())
...
func save_slot(slot: int = -1) -> bool:
	...
	if not _valid(s):
		return false
	var ok := SaveWriter.write(_slot_modules.values(), _slot_path(s), game_version)
...
func load_slot(slot: int = -1) -> bool:
	...
	if not _valid(s):
		return false
	var ok := SaveWriter.read(_slot_path(s), _slot_modules.values())
	if ok:
		current_slot = s
		slot_changed.emit(s)
